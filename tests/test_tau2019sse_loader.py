...
def test_tau2019sse_init():
    ...
    assert len(a.micarray_clip_ids['Eigenmike']) == len(soundata.initialize('tau2019sse',data_home='~/').clip_ids)/2 # because soundata has clip_ids for each format A and B
    assert len(a.clips_list) == len(soundata.initialize('tau2019sse',data_home='~/').clip_ids)/2 # because soundata has clip_ids for each format A and B
...
def test_tau2019sse_get_audio_numpy():
    ...
    a = tau2019sse_loader.tau2019sse(download=False,data_home='tests/resources/datasets/tau2019sse')
    A = a.get_audio_numpy('dev/split1_ir0_ov1_1')
    B = a.get_audio_numpy('dev/split1_ir0_ov1_1',fmt='B')

    Al = librosa.load('tests/resources/datasets/tau2019sse/mic_dev/split1_ir0_ov1_1.wav',sr=24000,mono=False)[0]
    Bl = librosa.load('tests/resources/datasets/tau2019sse/foa_dev/split1_ir0_ov1_1.wav',sr=24000,mono=False)[0]
