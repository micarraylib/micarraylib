...
def test_eigenscape_init():
    ...
    assert len(a.micarray_clip_ids['Eigenmike']) == len(soundata.initialize('eigenscape',data_home='~/').clip_ids)
    assert len(a.clips_list) == len(soundata.initialize('eigenscape',data_home='~/').clip_ids)
...
def test_eigenscape_get_audio_numpy():
    ...
    a = eigenscape_loader.eigenscape(download=False,data_home='tests/resources/datasets/eigenscape')
    A = a.get_audio_numpy('Beach.1')

    Al = librosa.load('tests/resources/datasets/eigenscape/Beach.1.wav',sr=48000,mono=False)[0]
...
def test_eigenscape_raw_get_audio_numpy():
    ...
    a = eigenscape_loader.eigenscape_raw(download=False,data_home='tests/resources/datasets/eigenscape_raw')
    A = a.get_audio_numpy('Beach-01-Raw')

    Al = librosa.load('tests/resources/datasets/eigenscape_raw/Beach-01-Raw.wav',sr=48000,mono=False)[0]
