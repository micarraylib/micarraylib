...
def test_tau2020sse_nigens_init():
    ...
    # TODO: analyze why the atol is needed
    assert np.allclose(
        np.mean(np.array([c for c in b.values()]), axis=0), [0, 0, 0], atol=1e-4
    )
    assert (
        len(a.micarray_clip_ids["Eigenmike"])
        == len(soundata.initialize("tau2020sse_nigens", data_home="~/").clip_ids) / 2
...
def test_tau2020sse_nigens_get_audio_numpy():
    ...
    a = tau2020sse_nigens_loader.tau2020sse_nigens(
        download=False, data_home="tests/resources/datasets/tau2020sse_nigens"
    )
    A = a.get_audio_numpy("dev/fold1_room1_mix001_ov1")
    B = a.get_audio_numpy("dev/fold1_room1_mix001_ov1", fmt="B")

    Al = librosa.load(
        "tests/resources/datasets/tau2020sse_nigens/mic_dev/fold1_room1_mix001_ov1.wav",
        sr=24000,
        mono=False,
    )[0]
    Bl = librosa.load(
        "tests/resources/datasets/tau2020sse_nigens/foa_dev/fold1_room1_mix001_ov1.wav",
        sr=24000,
        mono=False,
    )[0]
