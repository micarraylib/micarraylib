...
def test_tau2021sse_nigens_init():
    ...
    assert len(a.micarray_clip_ids['Eigenmike']) == len(soundata.initialize('tau2021sse_nigens',data_home='~/').clip_ids)/2 # because soundata has clip_ids for each format A and B
    assert len(a.clips_list) == len(soundata.initialize('tau2021sse_nigens',data_home='~/').clip_ids)/2 # because soundata has clip_ids for each format A and B
...
def test_tau2021sse_nigens_get_audio_numpy():
    ...
    a = tau2021sse_nigens_loader.tau2021sse_nigens(download=False,data_home='tests/resources/datasets/tau2021sse_nigens')
    A = a.get_audio_numpy('dev/dev-train/fold1_room1_mix001')
    B = a.get_audio_numpy('dev/dev-train/fold1_room1_mix001',fmt='B')

    Al = librosa.load('tests/resources/datasets/tau2021sse_nigens/mic_dev/dev-train/fold1_room1_mix001.wav',sr=24000,mono=False)[0]
    Bl = librosa.load('tests/resources/datasets/tau2021sse_nigens/foa_dev/dev-train/fold1_room1_mix001.wav',sr=24000,mono=False)[0]
